...
// AddressField is a custom type that can handle both []string and map[string]string formats
// for the address field in JWT tokens
type AddressField []string

// UnmarshalJSON implements custom JSON unmarshaling for AddressField
// It handles both []string and map[string]string formats
func (a *AddressField) UnmarshalJSON(data []byte) error {
	...
	if err := json.Unmarshal(data, &strSlice); err == nil {
		*a = AddressField(strSlice)
		return nil
	}

	// If that fails, try to unmarshal as map[string]string
	var addressMap map[string]string
	if err := json.Unmarshal(data, &addressMap); err == nil {
		// Convert map to []string by extracting the "formatted" field if available and not empty
		// Otherwise, create a slice with all non-empty values
		var result []string
		if formatted, ok := addressMap["formatted"]; ok && formatted != "" {
			result = append(result, formatted)
		} else {
			// Collect all non-empty values from the map in a consistent order
			// Priority order: street_address, locality, region, postal_code, country
			priorityKeys := []string{"street_address", "locality", "region", "postal_code", "country"}
			for _, key := range priorityKeys {
				if value, ok := addressMap[key]; ok && value != "" {
					result = append(result, value)
				}
			}
			// Add any remaining keys that weren't in the priority list
			for key, value := range addressMap {
				if value != "" {
					...
					if !isPriorityKey && key != "formatted" {
						result = append(result, value)
					}
				}
			}
		}
		*a = AddressField(result)
		return nil
	}

	// If both fail, return the error from the map unmarshal attempt
	return json.Unmarshal(data, &addressMap)
}

// MarshalJSON implements custom JSON marshaling for AddressField
func (a AddressField) MarshalJSON() ([]byte, error) {
